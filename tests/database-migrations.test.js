...
test('fresh database migrates to the latest schema with foreign keys enabled', () => {
  ...
  try {
    ...
    assert.ok(database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'work_design_bindings'").get())
    assert.ok(database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'work_design_packages'").get())
    assert.ok(database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'work_design_objects'").get())

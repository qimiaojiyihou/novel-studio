...
test('v21→v27 preserves pinned historical packs, runs sequentially and cascades new data', () => {
  const db = new DatabaseSync(':memory:')
  ...
  runMigrations(db)
  assert.deepEqual(db.prepare('SELECT version FROM schema_migrations WHERE version>=22 ORDER BY version').all().map(r=>r.version), [22,23,24,25,26,27])

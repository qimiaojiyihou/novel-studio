...
> 文档版本：v0.3 长篇创作基线
> 更新日期：2026-08-23
> 当前状态：桌面骨架、项目与章节管理闭环、正文审阅链路、故事规划中心、知识与连续性、模型诊断、长篇上下文，以及提示词与文风管理中心已经完成；导入导出与发布工程仍按后续阶段推进。

> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v10、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、连接测试、DeepSeek 高级参数、JSON 自定义请求配置、版本化 PromptCompiler、项目/卷/章节三级结构化文风、可叠加提示词插件、创作模板 v2、四类局部重写预设、最终编译预览、生成提示词快照、OpenAI 兼容适配器，以及故事基础、人物、世界观、总纲、分卷、章节规划、事实库、时间线、伏笔账本、连续性检查和长篇上下文。规划字段、章节卡、场景计划与正文均经过候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。
...
schema v10 已把任务提示词从模型适配器拆入统一 `PromptCompiler`，并完成第一批网络研究成果接入：

- 六类内置任务模板以模板版本和全局绑定保存，内置模板只负责任务协议，不与供应商请求代码混合；
...
schema v8 建立 `prompt_templates`、`prompt_template_versions`、`prompt_bindings`、`style_profiles` 和 `generation_records`；schema v9 增加 `prompt_addons`、`prompt_addon_versions` 和 `prompt_addon_bindings`；schema v10 保留创作模板 v1、启用 v2，并补入首批连续性、因果和视角插件。生成记录保留完整提示词快照，不依赖模板或插件后续是否升级，旧结果始终可追溯。
...
阶段 2 后续增强项：关系图、跨卷情节弧视图、地点视图和从正文自动提取知识，将在当前知识与连续性基础上继续建设，避免规划数据与连续性数据形成两套来源。

...
> 文档版本：v0.3 长篇创作基线
> 更新日期：2026-08-24
> 当前状态：桌面骨架、项目与章节管理闭环、正文审阅链路、故事规划中心、知识与连续性、章后状态提取、知识条目逐条签批与 AI 连续性审计、模型诊断、长篇上下文，以及提示词与文风管理中心已经完成；导入导出与发布工程仍按后续阶段推进。

> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v12、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、连接测试、DeepSeek 高级参数、JSON 自定义请求配置、版本化 PromptCompiler、项目/卷/章节三级结构化文风、可叠加提示词插件、创作模板 v2、四类局部重写预设、章后状态提取、AI 连续性审计、最终编译预览、生成提示词快照、OpenAI 兼容适配器，以及故事基础、人物、世界观、总纲、分卷、章节规划、事实库、时间线、伏笔账本、连续性检查和长篇上下文。规划字段、章节卡、场景计划、正文、章后状态、状态拆分知识条目与审计结果均经过候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。
...
当前 schema v12 已把任务提示词从模型适配器拆入统一 `PromptCompiler`，并完成前两批网络研究成果接入：
...
阶段 2 后续增强项：关系图、跨卷情节弧视图和地点视图将在当前知识与连续性基础上继续建设。已接受状态快照拆成可编辑知识条目的闭环已经完成，规划数据与连续性数据继续共用正式知识来源。

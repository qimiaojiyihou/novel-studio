...
> 文档版本：v0.2 实现基线
> 更新日期：2026-08-20
> 当前状态：桌面骨架、项目与章节管理闭环、正文审阅链路、故事规划中心和知识与连续性基础已经完成；导入导出与发布工程仍按后续阶段推进。

> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v6、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、OpenAI 兼容适配器，以及故事基础、人物、世界观、总纲、分卷、章节规划、事实库、时间线、伏笔账本和连续性检查。所有规划字段均可手工编辑或生成 AI 候选，候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。

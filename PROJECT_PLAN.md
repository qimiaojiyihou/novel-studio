...
> 文档版本：v0.3 长篇创作基线
> 更新日期：2026-08-24
> 当前状态：桌面骨架、项目与章节管理闭环、正文审阅链路、故事规划中心、人物关系图、地点视图、跨卷情节弧、知识与连续性、章后状态提取、知识条目逐条签批与 AI 连续性审计、模型诊断、长篇上下文、提示词与文风管理中心，以及首版导入导出与项目备份恢复已经完成；发布工程仍按后续阶段推进。

> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v14、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、连接测试、DeepSeek 高级参数、JSON 自定义请求配置、版本化 PromptCompiler、项目/卷/章节三级结构化文风、可叠加提示词插件、创作模板 v2、四类局部重写预设、章后状态提取、AI 连续性审计、最终编译预览、生成提示词快照、OpenAI 兼容适配器、TXT/Markdown/Word 文稿交换、带 SHA-256 完整性校验的 JSON 项目备份，以及故事基础、人物、结构化关系图、世界观、地点视图、总纲、分卷、跨卷情节弧、章节规划、事实库、时间线、伏笔账本、连续性检查和长篇上下文。规划字段、章节卡、场景计划、正文、章后状态、状态拆分知识条目与审计结果均经过候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。
...
当前 schema v14 已把任务提示词从模型适配器拆入统一 `PromptCompiler`，并完成前两批网络研究成果接入：
...
- 十七个内置提示词插件与自定义插件可以按任务和范围启停，真实生成和预览使用相同的解析顺序；
- 章节卡、场景计划和正文内置模板升级为 v2，旧版内容继续保存在不可变模板版本中；
- 局部重写使用对白有效化、减少解释式叙述、内在冲突强化和节奏压缩四个预设，预设标识进入 schema 3 生成快照；
...
schema v8 建立 `prompt_templates`、`prompt_template_versions`、`prompt_bindings`、`style_profiles` 和 `generation_records`；schema v9 增加 `prompt_addons`、`prompt_addon_versions` 和 `prompt_addon_bindings`；schema v10 保留创作模板 v1、启用 v2，并补入首批连续性、因果和视角插件；schema v11 增加 `knowledge_candidates`，并允许保留来源为 AI 的连续性检查；schema v12 增加 `knowledge_item_candidates`，把状态快照拆出的事实、时间线和伏笔置于作者逐条签批之后，并为已确认条目保留 AI 来源；schema v13 增加 `character_relationships`，以人物卡外键维护方向、趋势、状态和真实张力；schema v14 增加 `story_arcs` 与 `story_arc_beats`，让情节线以关键变化节点跨卷、跨章节组织。生成记录保留完整提示词快照，不依赖模板或插件后续是否升级，旧结果始终可追溯。
...
当前实现中，TXT、Markdown 和 Word 用于正文交换；JSON 用于完整项目备份。项目备份包含正文、版本、规划、知识与连续性、上下文、项目提示词绑定、自定义提示词和生成记录，不包含模型档案、任务路由或 API Key。恢复时校验格式版本与 SHA-256 摘要，并创建带新 ID 的“恢复副本”，原项目保持不变。
...
当前已完成模型档案、任务路由、OpenAI 兼容流式请求、Go/内置双网关、取消、基础连接测试和 DeepSeek 高级参数透传。后续继续补供应商能力探测、重试队列和完整生成记录。
...
1. 完整供应商能力探测、失败重试队列和生成记录浏览界面；
2. 导入章节切分预览、冲突映射与自动备份策略；
3. CI、跨平台安装包、签名与开源文档；
4. 网页端服务协议与部署方案评估。

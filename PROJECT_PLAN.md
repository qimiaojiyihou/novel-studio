...
> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v9、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、连接测试、DeepSeek 高级参数、JSON 自定义请求配置、版本化 PromptCompiler、项目/卷/章节三级结构化文风、可叠加提示词插件、最终编译预览、生成提示词快照、OpenAI 兼容适配器，以及故事基础、人物、世界观、总纲、分卷、章节规划、事实库、时间线、伏笔账本、连续性检查和长篇上下文。规划字段、章节卡、场景计划与正文均经过候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。
...
schema v9 已把任务提示词从模型适配器拆入统一 `PromptCompiler`，并完成可视化管理闭环：
...
- 十个内置提示词插件与自定义插件可以按任务和范围启停，真实生成和预览使用相同的解析顺序；
...
schema v8 建立 `prompt_templates`、`prompt_template_versions`、`prompt_bindings`、`style_profiles` 和 `generation_records`；schema v9 增加 `prompt_addons`、`prompt_addon_versions` 和 `prompt_addon_bindings`。生成记录保留完整提示词快照，不依赖模板或插件后续是否升级，旧结果始终可追溯。

...
> 文档版本：v0.4 模型可靠性基线
> 更新日期：2026-08-24
> 当前状态：桌面骨架、项目与章节管理闭环、正文审阅链路、故事规划中心、人物关系图、地点视图、跨卷情节弧、知识与连续性、章后状态提取、知识条目逐条签批与 AI 连续性审计、模型能力探测、安全自动重试、生成记录与人工重试、长篇上下文、提示词与文风管理中心，以及首版导入导出与项目备份恢复已经完成；发布工程仍按后续阶段推进。

> 当前实现：Vue 3 + JavaScript + Vite + Electron 桌面应用已接入 SQLite schema v15、CodeMirror 6 正文编辑器、Monaco 差异确认、Go 服务启动管理、模型配置与任务路由、四项供应商能力探测、生成前安全自动重试、生成记录浏览与人工重试、DeepSeek 高级参数、JSON 自定义请求配置、版本化 PromptCompiler、项目/卷/章节三级结构化文风、可叠加提示词插件、创作模板 v2、四类局部重写预设、章后状态提取、AI 连续性审计、最终编译预览、生成提示词快照、OpenAI 兼容适配器、TXT/Markdown/Word 文稿交换、带 SHA-256 完整性校验的 JSON 项目备份，以及故事基础、人物、结构化关系图、世界观、地点视图、总纲、分卷、跨卷情节弧、章节规划、事实库、时间线、伏笔账本、连续性检查和长篇上下文。规划字段、章节卡、场景计划、正文、章后状态、状态拆分知识条目与审计结果均经过候选确认后才进入正式内容；真实模型未配置时自动使用带明确提示的 Mock 回退。
...
当前 schema v15 已把任务提示词从模型适配器拆入统一 `PromptCompiler`，并完成前两批网络研究成果接入：

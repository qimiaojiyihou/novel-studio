...
import { PROFILE_DEFAULTS } from './context-repository.js'
...
export function readCreativeSnapshot(db, projectId) {
  ...
  const continuityChecks = rows('continuity_checks')
  const contextProfile = db.prepare('SELECT * FROM context_profiles WHERE project_id=?').get(projectId) || {
    project_id:projectId,
    max_context_chars:PROFILE_DEFAULTS.maxContextChars,
    recent_chapter_count:PROFILE_DEFAULTS.recentChapterCount,
    relevant_chapter_count:PROFILE_DEFAULTS.relevantChapterCount,
    knowledge_limit:PROFILE_DEFAULTS.knowledgeLimit,
    chapter_summary_chars:PROFILE_DEFAULTS.chapterSummaryChars,
  }
  ...
  const sourceDigest = creativeDigest(clean({ project, chapters, planning, styles:structuredStyles, styleSamples, protections,
    contextProfile, knowledge:knowledge.filter(row => ['manual','ai'].includes(row.source_type)), memories:memories.filter(row => row.confirmed) }))
  ...
  return { project, sourceDigest, chapters, planning, knowledge, continuityChecks, contextProfile, memories, styles, styleSamples, protections, finalizations, runs, candidates }
}

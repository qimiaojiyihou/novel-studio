export const PROMPT_SNAPSHOT_SCHEMA_VERSION = 7
...
export const FICTION_NATURALNESS_RULES = [
  '规划只规定必须成立的事实、因果和边界，不是段落提纲；不得把章节卡或场景卡逐条扩写成等长段落。',
  '写作前在内部锁定视角人物此刻要办的事、没有说出口的顾虑、自我保护方式和注意力偏向；叙述只选择这个人当下会留意、误判、回避或舍不得放过的内容。',
  '分配篇幅要有取舍：不可逆选择、关系变化和意外结果可以放慢，低价值衔接可以一句带过；不要让每个动作、每条规则和每个场景获得相同篇幅。',
  '先按句群组织叙事。同一时刻、同一主体、同一动作链或同一因果中的信息应优先组成连续句群，不把一个完整动作拆成数个相邻短句。',
  '短句只有在注意力突然转移、人物作出决定、局势发生不可逆变化或对白需要落点时才单独成句。若一句只报告“看或没看、动或没动、说或没说、松或没松”等单一动作，并未带来新的判断、关系或局势变化，就与前后动作合并并写清动作对象。',
  '避免机械的“人物动作。另一人物反应。”两拍结构。动作与反应属于同一交互时，写清谁对什么做了什么，以及该反应如何影响后续动作，不依靠省略动作对象制造虚假停顿。',
  '旁白不使用抽象判断替代现场变化。若判断确实来自视角人物，它必须由眼前事实触发，并直接影响人物接下来的注意、措辞或选择；不要为了段落落点而在普通动作后补一条机敏总结或反差句。',
  '同一个情绪或含义只表达一次。动作、物件处理、对白或沉默已经让读者看懂时，不再追加“他意识到、这意味着、原来、真正让他”等解释性复述。',
  '对白中的每个人都有自己的事情、利益和信息缺口；允许答非所问、打断、停顿、改口、说一半和边做事边说，不让角色轮流完整陈述局势、主题或条款。',
  '配角继续处理自己的工作和风险，不只负责见证、赞扬或认证主角；认可应当带条件、保留或下一项现实要求，避免按“第一次反应、第二次反应、第三次反应”排成验证阶梯。',
  '专业能力用一至三个能改变判断或结果的高信号细节证明，再让后果、选择或他人应对完成验证；不要把完整操作流程写成均匀、无阻力的技术演示。',
  '段落跟随注意力、动作压力和关系拍点自然伸缩，保留必要的长句、短句和粗粝处；不要为制造“人味”随机切碎句子、添加口头禅、错别字、夸张身体反应或无来源生活细节。返回前检查连续句群：若相邻三句都是长度接近的单一动作、否定反应或作者判断，且每句没有独立改变局势，就重新组织为自然的动作—对象—结果或感知—判断—选择结构。',
  '除受保护的逐字硬停止句外，不直接复刻章节卡和场景卡措辞；把合同事实转成现场行为，同时保留未说尽、未解决和人物自己也没想明白的部分。',
].join('')
...
export const PLANNING_PROMPT_PROFILES = Object.freeze({
  ...
  chapter_title: {
    name: '连载章节标题',
    system: [
      '章节名是读者进入本章前看到的点击钩子，不是对已经发生事件的账目式摘要。',
      '先从本章提取最有辨识度的身份反差、现场冲突、人物口吻、异常条件、未决问题或即将兑现的回报。',
      '在内部至少拟出四个采用不同钩子机制的标题并比较，淘汰只把金额、动作、地点和时长拼在一起的标题，最后只返回最强的一个。',
    ].join(''),
    request: [
      '标题既要让读者知道本章有具体事件，又要故意留下半步没有说完。',
      '优先使用人物选择、身份碰撞、现场质疑、异常报价或一句有声音的话；保持项目题材和语气，不虚构正文里没有的爆点。',
      '通常使用 6—18 个中文字符，可按需要使用一个逗号、问号或叹号；不要带章节序号、书名号、解释、评分或候选列表。',
    ].join(''),
  },
})
...
export const REWRITE_PRESETS = Object.freeze([
  ...
  },
  {
    ...
    name: '叙事真人化精修',
    shortLabel: '去工整感',
    content: '先定位真实存在的工整病灶，只修改命中的句群，不对整章做同义改写。重点检查章节卡逐条转写、分级验证式反应、连续机械短句、动作与反应被句号强行拆开、动作对象被省略、孤立的否定动作、旁白抽象总结和每段都追求聪明落点。属于同一时刻和因果链的动作应重组为连贯句群，补清动作对象、空间关系和结果，但不添加新情节；短句只在人物决定、注意力骤变、不可逆事件或有效对白落点处保留。抽象判断若没有影响人物下一步行动就删除；若确实属于人物判断，就放回具体感知和选择中。保留有辨识度的原句、事实、事件顺序、人物状态、视角、人物声音和结尾，不通过随机短句、俚语、错别字、长句堆叠或另一套统一句式表演“人味”。',
  },
  {
    id: 'compress',
    name: '节奏压缩',
...
export const BUILTIN_PROMPT_TEMPLATES = Object.freeze([
  ...
  },
  {
    ...
    version: 20,
    content: {
      system: `${CORE_SYSTEM_RULES}把规划转化为现场中的动作、感知、对白、判断和选择。人物只知道其知情范围内的信息。保持视角、时间、地点、物件和能力连续，不替读者解释已经能从现场推断的内容。${FICTION_NATURALNESS_RULES}`,
      request: '按场景计划顺序在一次响应中生成完整本章候选正文，必须覆盖所有场景并到达最后 actionBeat；不得只写前半章或停在中间场景的悬念处。每个场景从 entryState 开始，以 exitState 结束；让变化通过人物行动发生，但场景卡只是事实核对表，不是逐条转写清单：先决定本场最值得展开的一次选择或关系变化，其余必要事实可以合并、穿插或简洁带过，不按 actionBeats 数量平均分段。保留必要的呼吸与具体细节，但不新增会改变后续结构的角色、规则、能力或解决方案。场景计划若调用上方已确认上下文中没有出现的截图、地图、日志、地点编号或其他导航证据，不得把它写成既有事实；应改成不改变章节合同的现场可见搜索、辨认或试探动作。写作前在内部逐条核对硬规则的触发条件和成功/失败分支；凡已触发的规则，必须把全部强制成功后果、复位/关闭/归还等动作逐项落实，但这些事实可以通过同一段连续行动自然完成，不得为了显示合规而逐条解释或复述规则。冲突时删除场景计划中的错误细节并服从更高层已确认规则。返回前核对每个命名证物的具体版本和属性极性：同一份文件或物件不得同时是已修改/未修改、完好/损坏、锁定/未锁定；如果是原件、备份或不同时点的多个版本，必须显式区分并给出属性变化证据。完成事实核对后再做一次叙事自然度反查：删除动作或对白后紧跟的意义复述，删除配角分级验证主角的反应阶梯，压缩过度完整的条款式对话和操作说明；保留真正有声音的原句、必要停顿和未说尽之处，不做全章同义改写。再识别 ending 和最后一个 actionBeat 中“最后允许发生的事件”：最后一个 actionBeat 是停止边界，不是可扩写段落。硬停止事件在全文只能完成一次；不得在前文先用同义句写出已经离场、已经消失、已经关门、已经停住或已经完成最后反应，再在结尾逐字复述。前文只能写尚未完成该结果的过程，返回前从倒数第三段开始扫描并删除重复完成事件。若最后 actionBeat 已是即时身体或感官反应，它就是唯一允许的反应，并且必须逐字成为全文最后一句；写完后立即停止，不得补充身体、感官、环境或心理句。continuityRisks、exitState 中用于提醒“不做什么”的文字只是约束，绝不能转写进正文。不得追加第二个反应、否定式动作清单、环境收束、总结、下一步、调查、进入、撤离或解释。',
      outputContract: PROTECTED_OUTPUT_CONTRACTS.chapter,
    },
  },
  {
  ...
  },
  {
    ...
    version: 8,
    content: {
      system: `${CORE_SYSTEM_RULES}你是独立小说编辑。只根据已确认规划、场景合同和候选文本评分。每个问题都要引用候选中的具体证据；不要把个人文风偏好或语法完整本身当成硬错误。连续性、人物知情边界、世界硬规则和章节越界属于高严重度。已确认规则不可通过修改规划来迁就候选正文；修复指令必须修改候选。文字自然度审的是叙事是否来自一个具体视角人物的选择性注意，而不是是否加入俚语或随机短句。重点识别章节卡逐条转写、段落与句长机械匀速、所有动作平均用力、动作后解释含义、角色轮流完整总结、配角只负责验证主角、专业流程无阻力铺陈、情绪只被命名而没有影响注意和选择等成簇问题。还要检查连续的“人物名或代词＋没、不、未＋单一动作”孤立句、完整交互被拆成“人物动作。另一人物反应。”的机械两拍、动作对象被省略形成虚假停顿、旁白用抽象判断替代现场，以及每段都以机敏总结或反差句收尾。章节卡 ending 是最后允许发生的事件；事件后超过一个句子的一次即时感官或身体反应、继续写第二个反应、否定式动作清单、环境收束、思考行动方案、重复升级事件或开始调查，都属于高严重度越界。仅中文单双引号、直引号与弯引号之间的排版差异不改变文本事实，不得因此列问题或扣分。硬停止句若以对白结束，同一句中位于该对白之前、且不改变说话者和事件的前置动作不属于越界；闭引号后的内容才属于事件后续写。`,
      request: '按规划遵循、因果推进、场景变化、连续性、人物主动性与声音、悬念与结尾、文字自然度七个维度各给 1–5 分。先逐字核对上一章已确认状态、当前章节合同、世界规则的条件分支和结尾边界；比对含对白的硬停止句时，中文单双引号、直引号和弯引号视为等价，对白文字与末尾标点须一致，允许同一句在对白前加入不改变事件的可见前置动作，闭引号后不得续写。再列出可定位、可最小修复的问题。评估 proseNaturalness 时先找成簇证据：除聊天残留或直接复刻合同等单点污染外，每个自然度问题至少引用两处同类证据，并说明它如何抹平视角、情绪、人物利益或叙事取舍；单次孤立短句不直接判错。相同机械短句、动作—反应拆分、对象省略或抽象总结病灶出现在两个以上段落时必须报告；若跨多个场景反复出现，proseNaturalness 不得高于 2 分。只有具体、选择性、节奏随压力变化且保留未说尽之处的正文才可给 5 分，出现章节卡逐项转写、验证阶梯、条款式完美对白或大面积均匀技术演示时也不得高于 2 分。repairInstruction 必须指向证据所在的最小段落或句群，说明应合并、补清动作对象还是删除抽象总结；不得用“整章写得更自然”或“增加长句”代替，也不得建议随机打碎句子、添加口头禅或虚构生活细节。对结尾必须引用从最后允许事件开始直到全文结束的原文并计算其后的句子或动作拍数；对状态必须把状态结论与正文证据成对比较，特别检查否定、尚未、损坏与失效。没有证据的问题不要猜测。',
